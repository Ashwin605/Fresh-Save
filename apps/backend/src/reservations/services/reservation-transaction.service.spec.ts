...
describe('ReservationTransactionService (Concurrency)', () => {
  ...
  describe('createReservation', () => {
    ...
    it('should correctly accumulate reservedQuantity to prevent overselling on duplicate inventory IDs', async () => {
      const mockTxClient = {
        $queryRawUnsafe: jest.fn().mockResolvedValue([
          { id: 'inv-1', stockQuantity: 10, reservedQuantity: 0 }, // 10 available
        ]),
        inventory: { update: jest.fn().mockResolvedValue({}) },
        inventoryStockMovement: { create: jest.fn().mockResolvedValue({}) },
        reservation: { create: jest.fn().mockResolvedValue({ id: 'res-1' }) },
      ...
      // Sending two items of 6 quantity each (total 12) for the same inventoryId
      // First iteration: 10 available, requests 6, reserves it, updates memory to 6 reserved.
      // Second iteration: 4 available, requests 6. Should throw BadRequestException!
      await expect(
        service.createReservation({
          customerId: 'cust-1',
          storeId: 'store-1',
